...
public class StationScreenServiceImpl implements IStationScreenService {
	...
	public List<StationPowerMonthOutput> kWhStatisticByMonth(Integer id) {
		...
		return listResult.stream().map(item -> {
			...
			try {
				stationPowerMonthOutput.setPowerDate(DateUtil.stringToDate((String) item.get("month"), DateUtil.YYMM));
			} catch (Exception e) {
				e.printStackTrace();
			}
			stationPowerMonthOutput.setKwh(BigDecimal.valueOf((Double) item.get("kwh")).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
